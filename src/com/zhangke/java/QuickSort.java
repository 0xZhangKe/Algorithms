...
public class QuickSort {
     ...
     * 快速排序递归版
     */
    public static void sortUseRecursion(int[] arr) {
        doSortUseRecursion(arr, 0, arr.length - 1);
    }

    private static void doSortUseRecursion(int[] arr, int low, int high) {
        if (low < high) {
...
/*    public static void sortUseLoop(int[] arr) {
        ...
        int rightHigh = arr.length - 1;
        while (leftLow < leftHigh && leftHigh < rightLow && rightLow < rightHigh){
            int leftPivot = partition(arr, leftLow, leftHigh);
            leftHigh = leftPivot - 1;
            int rightPivot = partition(arr, rightLow, rightHigh);
            rightLow = rightPivot + 1;
        }
    }*/

    public static void sortUseLoop(int[] array){
